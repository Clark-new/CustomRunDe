...
public class EmoticonFragment extends BaseFragment {
    ...
    private EmotionIndicatorView pointGroup;//表情面板对应的点列表
    private int emotionType;

    @Override
    public int getRootResource() {
        return R.layout.layout_emotion;
    }

    @Override
    protected void initView() {
        super.initView();
        emotionViewPager = findViewById(R.id.id_emotion_layout);
        pointGroup = findViewById(R.id.id_point_group);
        //获取map的类型
        emotionType = getArguments().getInt(FragmentFactory.EMOTION_MAP_TYPE);
        initEmotion();
        initListener();
    }
    ...
    protected void initListener() {
        emotionViewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                pointGroup.playByStartPointToNext(oldPagerPos, position);
    ...
    private void initEmotion() {
        ...
        // item的间距
        int spacing = CommonUtils.dp2px(getActivity(), 12);
        // 动态计算item的宽度和高度
        ...
        //初始化指示器
        pointGroup.initIndicator(emotionViews.size());
        // 将多个GridView添加显示到ViewPager中
        emotionAdapter = new EmotionPagerAdapter(emotionViews);
    ...
    private GridView createEmotionGridView(List<String> emotionNames, int gvWidth, int padding, int itemWidth, int gvHeight) {
        ...
        // 给GridView设置表情图片
        EmotionGridViewAdapter adapter = new EmotionGridViewAdapter(getActivity(), emotionNames, itemWidth, emotionType);
        gv.setAdapter(adapter);
        //设置全局点击事件
        gv.setOnItemClickListener(GlobalOnItemClickManagerUtils.getInstance(getActivity()).getOnItemClickListener(emotionType));
        return gv;
    }
    ...
    public void onDestroy() {
        super.onDestroy();
        GlobalOnItemClickManagerUtils.relase();
    }
}
