...
public class TabPageIndicator extends HorizontalScrollView {
    ...
    private boolean isSameLine;// 设置导航线是否跟文字长度一至
...
//    = R.drawable.background_tab;//字体颜色的选择器
    ...
    public enum IndicatorMode {
        // 给枚举传入自定义的int值
        MODE_WEIGHT_NOEXPAND_SAME(0), // 平均分配，导航线宽度跟标题相等
        MODE_WEIGHT_NOEXPAND_NOSAME(1), // 平均分配，导航线宽度跟标题不相等
        MODE_NOWEIGHT_NOEXPAND_SAME(2),// 非平均分配，非扩展，导标宽度相等
        MODE_NOWEIGHT_NOEXPAND_NOSAME(3),// 非平均分配，非扩展，导标宽度不相等
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        TextView tab = (TextView) tabsContainer.getChildAt(0);
        int measuredWidth = tab.getMeasuredWidth();
        int length = tab.getText().toString().length();
        if (indicatorLineWidth == 0) {
            indicatorLineWidth = measuredWidth / length * 2;
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (currentIndicatorMode == IndicatorMode.MODE_NOWEIGHT_NOEXPAND_NOSAME) {
            ...
            int offset = (int) (((lineRight - lineLeft) - indicatorLineWidth) / 2);

//            int indicatorHeight = height - underlineHeight;

            canvas.drawRect(lineLeft + offset, height - indicatorHeight, lineRight - offset, height, rectPaint);
            canvas.drawCircle(lineLeft + offset, height - indicatorHeight / 2, indicatorHeight / 2, rectPaint);
            canvas.drawCircle(lineRight - offset, height - indicatorHeight / 2, indicatorHeight / 2, rectPaint);
        }
